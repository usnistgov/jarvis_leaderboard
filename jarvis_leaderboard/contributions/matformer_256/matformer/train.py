...
def train_dgl(
    ...
    if test_only:
        checkpoint_tmp = torch.load('/data/keqiangyan/NIST/matformer_jarvis_formation_energy_16_256_head2_3layer/checkpoint_500.pt')
        to_load = {
            "model": net,
            "optimizer": optimizer,
        ...
        f=open('AI-SinglePropertyPrediction-formation_energy_peratom-dft_3d-test-mae.csv','w')
        f.write('id,prediction\n')
        error = 0
        for i in range(len(dataset_test)):
            # print(dataset_test[i])
            jid = dataset_test[i]['jid']
            target = dataset_test[i]['target']
            # print(jid, target - (predictions[i] + mean_train))
            error += abs(target - (predictions[i] + mean_train))
            line=jid+','+str(predictions[i] + mean_train)+'\n'
            f.write(line)
        f.close()
        print("error:", error/len(dataset_test))
