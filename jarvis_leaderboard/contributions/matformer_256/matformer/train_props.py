...
def train_prop_model(
    ...
    if test_only:
        t1 = time.time()
        result = train_dgl(config, test_only=test_only, use_save=use_save, mp_id_list=mp_id_list,data_root=data_root)
        t2 = time.time()
        ...
        t1 = time.time()
        result = train_dgl(config, use_save=use_save, mp_id_list=mp_id_list, data_root=data_root)
        t2 = time.time()
