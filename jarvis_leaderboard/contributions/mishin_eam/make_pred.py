...
calculator = EAM(potential='Mishin-Ni-Al-2009.eam.alloy')
...
def get_eam_forces(atoms):
    atoms=atoms.ase_converter()
    atoms.calc = calculator
    forces = atoms.get_forces()
    energy = atoms.get_potential_energy()
    stress = 0 #atoms.get_stress()
...
for i in glob.glob('../../benchmarks/AI/MLFF/*energy*.zip'):
 if 'mlearn' in i and 'Ni' in i:
    ...
    for key,val in dat['test'].items():   
        ...
        energy,forces,stress=get_eam_forces(atoms)
